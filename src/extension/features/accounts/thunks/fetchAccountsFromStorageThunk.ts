...
>(ThunkEnum.FetchAccountsFromStorage, async (options, { getState }) => {
  ...
  if (!online) {
    logger.debug(
      `${ThunkEnum.AddStandardAssetHoldings}: extension offline, returning accounts from storage`
    );
  ...
  if (!network) {
    logger.debug(
      `${ThunkEnum.AddStandardAssetHoldings}: no network selected, returning accounts from storage`
    );
